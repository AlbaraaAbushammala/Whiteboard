...
(() => {
    ...
    const self = {
        ...
        store: config => new Promise((resolve, reject) => {
            ...
            self.helper.solveDependency(config.local).then(result => {
                config.local = result;
                        ...
                        if (input.type === 'checkbox') {
                            ...
                            [...input.querySelectorAll('option')].map(option => {
                                if (input.hasAttribute('multiple'))
                                    data[key].map(value => option.selected = self.helper.encodeObject(value) === (option.value ? option.value : option.innerHTML.trim()));
